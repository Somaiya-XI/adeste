...
struct ProfileView: View {
    ...
    var body: some View {
        ScrollView {
            ...
            .onAppear {
                userName = userManager.currentUser?.name ?? "My Name"
            }
            .onChange(of: showNameEditor) { _, isShowing in
                // Only save when popup closes (not on every keystroke)
                if !isShowing && !userName.isEmpty {
